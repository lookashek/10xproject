...
<div className="container mx-auto px-4 py-8 max-w-7xl">
  ...
  {!isLoading && flashcards.length === 0 && <EmptyState onAddClick={handleOpenCreateDialog} />}
...
<div className="flex items-center justify-between mb-6 gap-4">
  <Button onClick={onAddClick} variant="default" size="default">
  ...
  >
    ...
    <div className="absolute inset-0 backface-hidden">
      <div className="h-full p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 flex flex-col">
        ...
        <p className="flex-1 text-base text-gray-900 dark:text-gray-100 break-words">{flashcard.front}</p>
      </div>
    </div>
    ...
    <div className="absolute inset-0 backface-hidden rotate-y-180">
      <div className="h-full p-6 bg-blue-50 dark:bg-blue-900/20 rounded-lg shadow-md border border-blue-200 dark:border-blue-800 flex flex-col">
        ...
        <p className="flex-1 text-base text-gray-900 dark:text-gray-100 break-words">{flashcard.back}</p>
      </div>
    </div>
...
<div className="flex flex-col items-center justify-center py-16 px-4">
  ...
  <h2 className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-2">Brak fiszek</h2>
...
<Dialog open={open} onOpenChange={onClose}>
  <DialogContent className="sm:max-w-[600px]">
    ...
    <FlashcardForm initialData={flashcard} onSubmit={onSave} onCancel={onClose} isSaving={isSaving} />
  </DialogContent>
</Dialog>
```
...
function useFlashcardQueryParams() {
  ...
  const updateQueryParams = useCallback(
    (newParams: { page?: number; source?: SourceFilterValue }) => {
...
function useFlashcardList(initialData: FlashcardListResponse, page: number, source?: FlashcardSource) {
  ...
  const createFlashcard = useCallback(
    async (data: FlashcardFormData): Promise<void> => {
...
```tsx
{
  flashcards.length === 0 && sourceFilter !== "all" && (
    <div className="text-center py-16">
      <p className="text-gray-600 dark:text-gray-400 mb-4">Nie znaleziono fiszek dla wybranego filtra.</p>
...
<Layout title="Moje fiszki - 10x Cards">
  <FlashcardsView client:load initialData={initialData} initialPage={page} initialSource={source} />
</Layout>
```
