...
export function CharacterCounter({ current, min, max, isValid }: CharacterCounterProps) {
  const getStatusText = () => {
    if (current < min) {
  ...
  return (
    <div
      className={cn("flex justify-between text-sm mt-2", isValid ? "text-muted-foreground" : "text-destructive")}
      role="status"
      aria-live="polite"
      aria-atomic="true"
